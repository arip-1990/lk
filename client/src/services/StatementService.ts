...
export const statementApi = createApi({
    ...
    tagTypes: ['IStatement'],
    endpoints: (builder) => ({
        fetchStatements: builder.query<Pagination<IStatement>, {categoryId: number, pagination: {current: number, pageSize: number}}>({
            query: (args) => ({
...
export const { useFetchStatementsQuery, useAddStatementMutation, useUpdateStatementMutation, useDeleteStatementMutation } = statementApi;
